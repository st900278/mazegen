...
public abstract class Cell
{
    ...
    */
    ...
    public Cell(Maze maze, Position position)
    {
        ...
        neighbors = new Lazy<List<Cell>>(() =>
        {
            var list = new List<Cell>();
            Debug.WriteLine(allSides.Count);
    ...
    */
    //by lazy
    public Lazy<List<Cell>> neighbors;
     ...
     */
    public virtual Cell GetCellOnSide(ISide side){
    ...
    public bool HasSide(ISide side)
    {
     ...
     */
    public void OpenSide(ISide side)
    {
        ChangeSide(side, (v, s) => {
            return v & ~s;
        });
     ...
     */
    public void CloseSide(ISide side)
    {
        ChangeSide(side, (v, s) => {
            return v | s;
        });
    ...
    private void ChangeSide(ISide side, Func<int, int, int> operation) {
        var cell = GetCellOnSide(side);
        if (cell != null) {
            cell.value = operation(cell.value, side.opposite.Value.value);
        }
    ...
    {
        ISide side = FindSideOfCell(cell);
        if (side != null)
        {
           
            cell.value = cell.value & ~side.opposite.Value.value;
     ...
     */
    public ISide FindSideOfCell(Cell cell){
        if (cell.maze == maze)
        {
            foreach(var side in allSides)
            {
    ...
    public abstract List<ISide> allSides { get; set; }
